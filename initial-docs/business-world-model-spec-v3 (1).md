...
**Revision note (2026-05, v3.2).** Made the training/serving boundary explicit: the training loop is independent of Uni and reads flat Lance/Parquet shards on GPU, with no live queries. Uni's training-path role is limited to offline shard materialization, Locy label generation, and post-training model registration. Adds § 4.2.1 and § 7.6; updates § 9.1.
...
### 4.2.1 Training-path / serving-path boundary (Uni is not in the training loop)

**The training path is independent of Uni.** The training loop is a tight numerical computation that reads flat, pre-materialized tensor shards through a PyTorch `DataLoader` on GPU. It issues no Cypher, no Locy, and no live database queries. Placing Uni inside the loop would starve the GPUs and is an explicit anti-pattern.

Uni *bookends* the training path; it does not sit inside it. Its three training-path roles are all at the edges:

```
   RAW SOURCES
   (EDGAR, FRED, market, news, supply chain, ...)
        │
        ▼
   ┌─────────────────────────────────────────────────────────┐
   │ Uni  — system of record (data-prep + handoff only)      │
   │                                                         │
   │  (1) Ingest, point-in-time tagging (BTIC), entity       │
   │      reconciliation (UniId)                             │
   │  (2) Compute rule-defined labels/targets via Locy       │
   │      (so train labels == serve labels by construction)  │
   └───────────────────────────┬─────────────────────────────┘
                               │  one-time EXPORT job
                               │  (runs as-of queries, writes shards)
                               ▼
                  Flat Lance / Parquet shards on NVMe
                  (pre-baked windows, flattened graph-
                   neighbor features, precomputed labels)
                               │
   ┌───────────────────────────▼─────────────────────────────┐
   │            TRAINING  — no Uni dependency                │
   │                                                         │
   │  PyTorch DataLoader → GPU → JEPA / SSM loop →           │
   │  checkpoints → ONNX export                              │
   │                                                         │
   │  Pure tensor compute. No queries. Reproducible from a   │
   │  named Uni snapshot id captured at export time.         │
   └───────────────────────────┬─────────────────────────────┘
                               │  (3) register trained artifact
                               ▼
        uni-xervo alias  ──►  Uni SERVING path
        (bwm/firm-state-encoder/v1, ...)
```

```
   Training-path role          Where           In the loop?
   ──────────────────────────────────────────────────────────
   (1) Dataset materialization Offline export   No
       — PIT-correct shards     job (Uni → Lance)
   
   (2) Label / target          Offline, via     No
       generation               Locy during the
                                same export
   
   (3) Model registration      Post-training    No (handoff)
       — ONNX → uni-xervo       boundary
```

What this buys, versus materializing shards directly from raw sources: point-in-time correctness, entity reconciliation, and label definitions are computed *once*, in the system of record, and are guaranteed consistent between training and serving. The training loop never pays a query cost for any of it. The export job records the Uni snapshot id it read from, which is what makes a training run reproducible (and is the basis for the MREP version-locking story in § 8).

A consequence for engineering: the training environment (GPU cluster, PyTorch, custom CUDA) and the serving environment (Uni + Locy + uni-xervo) are separately deployable and have no shared runtime dependency. They communicate only through artifacts — shards in one direction, an ONNX model in the other.
...
### 7.6 Training-shard export (Uni → flat shards)

The ingest pipeline above loads data *into* Uni. Model training reads from flat shards exported *out* of Uni — the training loop never queries Uni directly (§ 4.2.1). The export is a separate, offline, one-time-per-dataset-version job:

```
   ┌────────────────────────────────────────────────────────────┐
   │                                                            │
   │  Pin a Uni snapshot id  (the dataset version)              │
   │    │                                                       │
   │    ▼                                                       │
   │  Run as-of windowed queries at that snapshot               │
   │    │  (VERSION AS OF <snapshot>; per entity-quarter,       │
   │    │   pull the full history window the encoder needs,     │
   │    │   PIT-correct by construction via BTIC)               │
   │    ▼                                                       │
   │  Flatten graph-neighbor features                           │
   │    │  (resolve the ~200 neighbors per entity ONCE here,    │
   │    │   bake them into the sample so the training loop       │
   │    │   needs no graph traversal)                           │
   │    ▼                                                       │
   │  Compute rule-defined labels/targets via Locy              │
   │    │  (e.g. "covenant breach within 4Q?", regime labels;   │
   │    │   same rules used at serve time → no train/serve      │
   │    │   label skew)                                         │
   │    ▼                                                       │
   │  Write Lance / Parquet shards to NVMe                       │
   │    │  (each shard records the source snapshot id)          │
   │    ▼                                                       │
   │  Hand shards to the GPU training environment               │
   │                                                            │
   └────────────────────────────────────────────────────────────┘
```
...
- **Reproducibility.** A training run is reproducible from the recorded snapshot id alone; re-running the export against the same snapshot yields identical shards. This is the basis for MREP version locking (§ 8.1).
- **No train/serve skew.** Labels and graph features are computed by the same Locy rules and the same graph that serving uses, just materialized ahead of time.
- **No loop-time cost.** All point-in-time logic, entity resolution, and neighbor resolution happen once, in the export, not per training step.
- **Environment independence.** The export job is the only component that touches both worlds. The GPU training environment has no Uni runtime dependency; the Uni serving environment has no PyTorch dependency.
...
```
   Output      Production-grade ingest pipeline; snapshotted
               ...
               × all modalities; offline training-shard
               export job (Uni → Lance/Parquet, § 7.6)
   ...
   Acceptance  • All records carry valid_from (BTIC),
               ...
               • Foundation Uni snapshot created and named
                 (BWM-FOUNDATION-v1)
               • Export job produces PIT-correct shards from
                 the named snapshot; re-running against the
                 same snapshot yields byte-identical shards
